...
export default function App() {
  ...
  return (
    ...
    {pokemonName && (<div> Edit <input placeholder={pokemonName[0].name}></input>   
    </div>
    )}
      ...
      {pokemon && pokemon[0].results.map((items) => (
        <div className="d-flex justify-content-center m-3" onClick={()=> dispatch(getPokemonNameAsync(items.name))} key={items.name}>{items.name ? items.name : "N/A"}<button to="/EditPoke">Edit</button></div>
      ))}
